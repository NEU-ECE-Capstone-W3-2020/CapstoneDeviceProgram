...
// Read callback
static void read_callback(TMR_Reader *_reader, const TMR_TagReadData *t, void *_cookie) {
    UNUSED(_reader);
    UNUSED(_cookie);
    ...
    if(t->data.len < EPC_OFFSET) {
        DPRINTF("Not enough data read!\n");
        return;
    }

    if(t->data.list[EPC_OFFSET] != VERSION_TAG) {
        DPRINTF("Wrong Version\n");
        return;
    }
...
int nano_init(Nano *nano) {
    ...
    if(status != TMR_SUCCESS) {
        DPRINTF("Error initializing nano\n");
        return -1;
    }
    ...
    if(status != TMR_SUCCESS) {
        DPRINTF("Error connecting to nano: %s\n", TMR_strerror(status));
        nano_cleanup(nano);
        return -1;
    }
    ...
    if(status != TMR_SUCCESS) {
        DPRINTF("Failed to get model: %s\n", TMR_strerror(status));
        nano_cleanup(nano);
        return -1;
    }
    if(strcmp(model.value, "M6e Nano") != 0) {
        DPRINTF("This is reporting its model as %s, not as a M6e Nano\n", model.value);
        nano_cleanup(nano);
        return -1;
    }
    ...
    if(status != TMR_SUCCESS) {
        DPRINTF("Failed to set baudrate: %s\n", TMR_strerror(status));
        nano_cleanup(nano);
        return -1;
    }
    ...
    if(status != TMR_SUCCESS) {
        DPRINTF("Failed creating readplan\n");
        nano_cleanup(nano);
        return -1;
    }
    ...
    if(status != TMR_SUCCESS) {
        DPRINTF("Failed to set readplan: %s\n", TMR_strerror(status));
        nano_cleanup(nano);
        return -1;
    }
    ...
    if(status != TMR_SUCCESS) {
        DPRINTF("Error setting region: %s\n", TMR_strerror(status));
        nano_cleanup(nano);
        return -1;
    }
    ...
    if(status != TMR_SUCCESS) {
        DPRINTF("Error getting maximum power: %s\n", TMR_strerror(status));
        nano_cleanup(nano);
        return -1;
    }
    status = TMR_paramGet(nano, TMR_PARAM_RADIO_POWERMIN, &power_min);
    if(status != TMR_SUCCESS) {
        DPRINTF("Error getting minimum power: %s\n", TMR_strerror(status));
        nano_cleanup(nano);
        return -1;
    }
    ...
    if(power > power_max){
        DPRINTF("Power outside of possible range, setting to max!\n");
        power = power_max; 
    } else if(power < power_min) {
        DPRINTF("Power outside of possible range, setting to min!\n");
        power = power_min;
    }
    ...
    if(status != TMR_SUCCESS) {
        DPRINTF("Error setting read power: %s\n", TMR_strerror(status));
        nano_cleanup(nano);
        return -1;
    }
    ...
    if(status != TMR_SUCCESS) {
        DPRINTF("Error setting write power: %s\n", TMR_strerror(status));
        nano_cleanup(nano);
        return -1;
    }
    ...
    if(status != TMR_SUCCESS) {
        DPRINTF("Error setting read listener callback: %s\n", TMR_strerror(status));
        nano_cleanup(nano);
        return -1;
    }
...
int nano_cleanup(Nano* nano) {
    ...
    if(status != TMR_SUCCESS) {
        DPRINTF("Error closing reader: %s\n", TMR_strerror(status));
        return -1;
    }
    return 0;
}
...
int nano_start_reading(Nano *nano) {
    ...
    if(status != TMR_SUCCESS) {
        DPRINTF("Failure to start reading: %s\n", TMR_strerror(status));
        return -1;
    }
    return 0;
}
...
int nano_stop_reading(Nano *nano){
    ...
    if(status != TMR_SUCCESS) {
        DPRINTF("Failure to stop reading: %s\n", TMR_strerror(status));
        return -1;
    }
    return 0;
}
